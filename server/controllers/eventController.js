    ...
    Event.findOne({ eventName: eventName })
      .then((foundEvent) => {
        if (foundEvent) {
          ...
          foundEvent.participants.push(participants[0]);
          foundEvent.availability[participants[0]] = availability;
...
eventController.getEvents = (req, res, next) => {
  ...
  User.findOne({username: userName})
    .then(user => {
        res.locals.eventsForUser = []
        const events = user.events // array of events
